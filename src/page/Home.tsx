...
const Home = () => {
  ...
  useEffect(()=>{
    const fetchNote =  async()=>{
      ...
      if(res?.data){
        ...
        const filteredNotes = res.data.allNotes.filter((note: Note) => note.isDeleted === 0);
        ...
        <div className="bg-violet-200 h-screen w-full overflow-hidden ">
          <div className="flex justify-end items-end mt-5 me-10 rounded-md  ">
            <div className=" flex border border-violet-700 rounded-md">
              <input type="text" className="w-full border-none" value={searchTerm} onChange={(e)=>setSearchTerm(e.target.value)}/>
          ...
          <div className="m-6 flex justify-between w-full">
            ...
            <div className="flex gap-2 p-3  text-black text-xl cursor-pointer" data-modal-target="default-modal"
    data-modal-toggle="default-modal" onClick={()=>setAddNote(true)}>
                <h1 className="mt-1"><FaFilePen/></h1>
  ...
  {notes && notes.length > 0 ? (
    notes?.map((note) => (
      ...
      >
        ...
        <div className="w-1/4 p-2 flex gap-3 justify-end">
            ...
            <FaEdit className="mt-1 text-xl cursor-pointer"  onClick={()=>handleEdit(note.id)} />
              <RiDeleteBinFill   className="mt-1 text-xl cursor-pointer" onClick={()=>handleMovetoBin(note.id)} />
    ...
    <div className="flex justify-center items-center mt-10">
      <img src={noNote} alt="" style={{width:'300px' , height:'400px'}} />
    </div>
  )}
</div>
